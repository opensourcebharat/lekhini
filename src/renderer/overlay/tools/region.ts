...
function make(ctx: ToolContext, p1: { x: number; y: number }, p2: { x: number; y: number }): RegionShape {
  return {
    kind: 'region',
    id: nextId('region'),
    p1,
...
export const region: Tool = (() => {
  let working: RegionShape | null = null;
  ...
  return {
    ...
    onUp(sample, ctx) {
      if (!working || !p1) return;
      const final: RegionShape = { ...working, p2: { x: sample.x, y: sample.y } };
      working = null;
      p1 = null;
      ctx.setDraft(null);
